...
onready var walk_audio: AudioStreamPlayer = $WalkSfx
onready var jump_audio: AudioStreamPlayer = $JumpSfx
...
export (AudioStream) var fire_sfx_primary
export (AudioStream) var fire_sfx_secondary
export (AudioStream) var fire_sfx_special
...
func _ready() -> void:
	initialize()
	weapon_manager.connect("weapon_fired", self, "_fire_audio")
...
enum ATT_TYPE { PRIMARY, SECONDARY, SPECIAL }

func _fire_audio(type: int) -> void:
	print("🔊 Reproduciendo audio para tipo:", type)
	match type:
		ATT_TYPE.PRIMARY:
			player_sfx.stream = fire_sfx_primary
		ATT_TYPE.SECONDARY:
			player_sfx.stream = fire_sfx_secondary
		ATT_TYPE.SPECIAL:
			player_sfx.stream = fire_sfx_special
	player_sfx.play()
