...
func _sum_hp(amount: int) -> void:
	...
	# Reproducir sonido si pierde vida
	if amount < 0 and damage_sfx:
		$SfxPlayer.stream = damage_sfx
		$SfxPlayer.play()
...
func _remove() -> void:
	...
	if death_sfx:
		$SfxPlayer.stream = death_sfx
		$SfxPlayer.play()

	# Esperar a que termine el sonido antes de eliminar la torreta
