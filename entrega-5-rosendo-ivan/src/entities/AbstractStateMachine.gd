...
func _initialize() -> void:
	for state_path in STATES_LIST:
		...
		if state.state_id == "":
			push_error("⚠️ El estado " + str(state.name) + " no tiene 'state_id' asignado.")
			continue
		
		states_map[state.state_id] = state
		print("✅ Registrando estado:", state.state_id)

	# Setea el personaje si está disponible
	if !character_path.is_empty():
		var ch: Node = get_node_or_null(character_path)
		if ch != null:
			self.character = ch  # esto llama a _set_character()
